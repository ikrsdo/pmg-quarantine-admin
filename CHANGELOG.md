...
- README screenshots replaced with a full set captured from demo mode
  (dashboard, all three quarantine types, Tracking Center, structured
  Message Events) instead of the smaller, older screenshot set.
...
- Demo mode's Tracking Center always showed "No recognized events - see
  the raw log for the full trail." instead of a structured Message
  Events timeline. The mock log lines generated by `mockPmgClient.js`
  had no leading timestamp/hostname prefix, so the frontend's log
  parser (`trackingLogEvents.js`) categorized every line as generic
  `Log`, which the timeline filters out entirely. Mock log lines now
  carry a proper classic-syslog prefix (`Mon D HH:MM:SS <host> ...`),
  matching the format the real PMG API returns.
