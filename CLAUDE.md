...
| Quarantine download | `GET /quarantine/download?mailid=` (raw filehandle - the `.eml` source, not a JSON envelope; note the id parameter is named `mailid` here, unlike every other quarantine endpoint's `id`) |
